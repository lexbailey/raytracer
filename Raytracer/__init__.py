...
def call_bound_method(instance, name, args=(), kwargs=None):
    """ This is a slight hack for getting multiprocessing to work """
    if kwargs is None:
        kwargs = {}
...
class RayTracer:
    ...
    def visible_lights(self, point):
        ...
        for light in self.lights:
            ...
            near_obj, _, _ = self.nearest_object_hit(point, ray_dir)
            if near_obj is None:
                visible.append(light)
        return visible

    def nearest_object_hit(self, ray_start, ray_dir):
        """ Get the first object hit by the given ray """
    ...
    def _trace_ray(self, ray_start, ray_dir, bounces=0, limit=10):
        ...
        if near_obj is not None:
            ...
            s += near_obj.colour_at_point(point, pid,
                                          lights, normalize(ray_start - point))
            ...
            if reflectiveness > 0:
                ...
                s += reflectiveness * \
                    self._trace_ray(point, reflect_ray,
                                    bounces=bounces + 1, limit=limit)
            if transmit > 0:
                s += transmit * \
                    self._trace_ray(
                        point, ray_dir, bounces=bounces + 1, limit=limit)
        return s.astype(int)

    def _trace_ray_from_pixel(self, pixel, limit=10):
        x, y = pixel
        # TODO make this actually not hard coded
        ray_start = np.array([(x - 300) / 6, -((y - 200) / 4), 0])
    ...
    def iter_all_pixels(self):
        """ returns an iterator that iterates over all (x, y) tuples in the result image """
        return product(range(self.px_size[0]), range(self.px_size[1]))

    def iter_regions(self, num_regions):
        """
            Returns an iterator that iterates over a num_regions subsets of iter_all_pixels
            iterator. The subsets are approximately equal in size.
        """
    ...
    def _render_region(self, region, limit=10):
        ...
        for pixel in region:
            output.append(
                tuple(self._trace_ray_from_pixel(pixel, limit=limit)))
            done_px += 1
            if done_px % 10 == 0:
                print("%.1f%%" % (100 * done_px / max_px))

    def render(self, filename, limit=10):
        """ Render the scene, save it in a file. Optinally limiting the ray bounce depth """
        renderpool = Pool()
        regions = [
            renderpool.apply_async(call_bound_method, args=(self, "_render_region", (region,),
            {"limit": limit}))
            for region in self.iter_regions(cpu_count())]
        renderpool.close()
        ...
        scale = 1
        out = self.viewport.resize(
            (self.px_size[0] * scale, self.px_size[1] * scale), resample=Image.NEAREST)
        out.save(filename)
